...
public class MouseManager extends GLFWMouseButtonCallback {
	...
	public void invoke(long window, int button, int action, int mods) {
		...
		if(button == GLFW_MOUSE_BUTTON_LEFT && action != GLFW_RELEASE) {
			...
			if(Window.indexFromId(window) == 0 && CursorManager.posY > 32) {
				LevelEditor.instance.getLevel().getTiles()[LevelEditor.instance.getActiveLayer()][(int) CursorManager.posX / (int) LevelEditor.instance.getScale()][(int) (CursorManager.posY - 32) / (int) LevelEditor.instance.getScale()] = Tile.fromCoord(LevelEditor.instance.getSelectedTile()[0], LevelEditor.instance.getSelectedTile()[1]);
			}
